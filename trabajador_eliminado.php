...
$sql = "SELECT c.*, e.estado FROM trabajador c JOIN estado e ON c.id_estado = e.id_estado WHERE c.id_estado = 1";
$stmt = $pdo->query($sql);
...
<body>
                        ...
                        <?php foreach ($trabajadores as $trabajador): ?>
                            ...
                            <td><?php echo htmlspecialchars($trabajador['estado']); ?></td>
                                <td>
                                    ...
                                    <a href="#" class="btn btn-danger btn-sm" onclick="confirmarEliminacion(<?php echo $trabajador['id_trabajador']; ?>)">Eliminar</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <script>
        ...
        function confirmarEliminacion(id_trabajador) {
            ...
            var confirmacion = confirm("¿Estás seguro de que deseas eliminar este conductor?");

            if (confirmacion) {
                // Si el usuario confirma, redirigir a un script PHP que actualizará el estado
                window.location.href = "eliminar_trabajador.php?id=" + id_trabajador;
            }
        }
    </script>
