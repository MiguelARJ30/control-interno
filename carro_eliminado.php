...
$sql = "SELECT c.*, e.estado FROM carro c JOIN estado e ON c.id_estado = e.id_estado WHERE c.id_estado = 1";
$stmt = $pdo->query($sql);
...
<body>
                ...
                <?php foreach ($vehiculos as $vehiculo): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="#" class="btn btn-danger btn-sm" onclick="confirmarEliminacion(<?php echo $vehiculo['id_carro']; ?>)">Eliminar</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <script>
        ...
        function confirmarEliminacion(id_carro) {
            ...
            var confirmacion = confirm("¿Estás seguro de que deseas eliminar este vehículo? (Solo se cambiará el estado a 'Eliminado')");

            if (confirmacion) {
                // Si el usuario confirma, redirigir a un script PHP que actualizará el estado
                window.location.href = "eliminar_carro.php?id=" + id_carro;
            }
        }
    </script>
